...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        elif isinstance(init_lora_weights, str) and init_lora_weights.startswith("loram"):
            with gather_params_ctx(self.get_base_layer().weight):
                self.loram_init(adapter_name, init_lora_weights)
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        scaling = torch.std(lora_B @ lora_A) / torch.std(Vr @ Uhr)
        print(scaling)
    ...
    def loram_init(self, adapter_name, init_lora_weights):
        ...
        if dtype not in [torch.float32, torch.float16, torch.bfloat16]:
            raise TypeError(
                "Please initialize LoRAM under float32, float16, or bfloat16. "
            )
        weight = weight.to(torch.float32)

        Uhr = fast_dst_matrix(self.r[adapter_name], weight.data.shape[1], device, torch.float32) 
        Vr = fast_dst_matrix(weight.data.shape[0], self.r[adapter_name], device, torch.float32) 

        variance_ratio = (torch.var(weight.data) / torch.var(Vr @ Uhr)).item()
        rho = math.log(self.r[adapter_name], min(*weight.shape))
        beta = math.pow(rho * variance_ratio, 1/4)

        lora_A = (Uhr * beta).contiguous()
        lora_B = (Vr * beta).contiguous()

        scaling = torch.std(lora_B @ lora_A) / torch.std(Vr @ Uhr)
        print(scaling.item(), self.scaling[adapter_name])
